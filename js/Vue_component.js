...
//    4) 사용자 정의 이벤트를 사용하여 폼 입력 컴포넌트 만들기
...
//      설명
//      1) v-model을 갖는 컴포넌트의 인스턴스는 template에, :value로 부모로부터 v-model로 보내진 값을 초기값으로 설정하며, @input이라는 커스텀이벤트를 생성할 수 있는데, 해당 커스텀이벤트로 input의 value를 조회하여 부모에게 방출해줄 수 있다.
new Vue({
  el : '#v-model1',
  data : {
    text : '초기값'
...
//    5) 컴포넌트의 사용자 정의 v-model 
//       대부분의 컴포넌트의 v-model은 value를 input의 초기값으로 사용하고, 사용자정의이벤트 input을 사용하지만, 체크박스나 라디오버튼과 같은 일부타입은 다른목적으로 value 속성을 사용할 수 있습니다.
//       model옵션을 사용하면 충돌을 피할 수 있습니다.
new Vue({
  el : '#v-model2',
  data : {
    boolean : true
...
//       이벤트를 방출하는 컴포넌트에서 $emit, 받는 컴포넌트에스 $on을 정의해놓는다.
...
//    1) 범위 컴파일
//       * 상위 템플릿(DOM)의 모든 내용은 상위 범위에서 컴파일 되며, 하위 템플릿의 모든 내용은 하위 범위에서 컴파일 됩니다.
//         즉, 컴포넌트에 v-show나 v-if등 디렉티브로 어떤 속성을 묶었을 때, 해당 속성이 상위 범위의 속성이면 작동하지만, 해당 컴포넌트 고유의 속성이면 작동되지 않습니다. 왜냐, 상위범위에서부터 렌더링 하는데, 상위범위는 하위컴포넌트의 속성을 알지 못하기 때문이다.
...
//  6. 기타
//     1) 태그의 ref속성과, $refs 호출을 통해 태그를 직접 조회할 수 있다.(document.querySelector처럼)  단, ref는 컴포넌트가 렌더링 된 이후에만 작동되며, 반응적이지 않는다.